...
const TextImgSlider = () => {
    ...
    return (
        <div className="relative mx-auto py-8 bg-gray-900">

            <article className='lg:px-[9%] md:pt-[9%] sm:px-[5%]'>
                <div ref={sliderRef} className="keen-slider">
                    {products.map((product, idx) => (
                        <div key={idx} className="keen-slider__slide">
                            <div className="flex flex-col md:flex-row items-center bg-gray-800 p-4 sm:p-6 rounded-lg shadow-md">
                                {/* Text content */}
                                <div className="flex-1 md:pr-6 text-center md:text-left">
                                    <h2 className="text-2xl sm:text-3xl mb-2 sm:mb-4 text-white">
                                        {product.title}
                                    </h2>
                ...
                {loaded && instanceRef.current && (
                    <>
                        ...
                        <button
                            ...
                            className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-gray-800 rounded-full p-2 shadow-md text-white"
                        >
                        ...
                        <button
                            ...
                            className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-gray-800 rounded-full p-2 shadow-md text-white"
                        >
                    ...
                    <div className="flex justify-center mt-4">
                        {[...Array(instanceRef.current.track.details.slides.length).keys()].map((idx) => (
                            <button
                                ...
                                className={`h-2 w-6 sm:w-8 rounded-full mx-1 ${currentSlide === idx ? 'bg-red-600' : 'bg-gray-500'}`}
                            ></button>
                        ))}
                    </div>
                )}
